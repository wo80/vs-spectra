...
extern "C"
{
	...
	EXPORT int spectra_di_ns(int which, int k, int ncv, int maxit, double tol,
		spectra_spmat *A, spectra_result *result);

	// Real non-symmetric standard problem - shift-and-invert mode
	EXPORT int spectra_di_ns_shift(int which,  int k, int ncv, int maxit, double tol, double sigma,
		spectra_spmat *A, spectra_result *result);
